...
Full checklist: [10_PRODUCTION_RUNBOOK.md](10_PRODUCTION_RUNBOOK.md) § Phase G ops.
...
- [x] ≥10 approved v2 rows with factual grounding checked  
- [x] Enough validated rows have embeddings for centroid job to write ≥1 centroid  
- [x] At least one `routing_mae_*.json` saved for metadata vs centroid comparison  
- [x] Injection and calibration remain OFF in prod until Phase J + F re-run  
...
Update [11_GO_NO_GO.md](11_GO_NO_GO.md) and dashboard overrides if GO. Do **not** flip flags without two stable eval runs for calibration.

### Next for F

Keep shadow ON; collect more predicts so stable-hash holdout includes more shadow rows; re-run eval twice; only then reconsider GO.
...
| [09_BUILD_PLAN.md](09_BUILD_PLAN.md) | Living build tracker |
| [10_PRODUCTION_RUNBOOK.md](10_PRODUCTION_RUNBOOK.md) | Staging ops checklists |
| [11_GO_NO_GO.md](11_GO_NO_GO.md) | Current prod decision |
